...
# merakibot's user ID in Slack: vlaue is assigned after the bot starts.
merakibot_id = None
...
def parse_bot_commands(slack_events):
    ...
    of command and channel. If its not found, then this function returns None,
    None"""
    
    for event in slack_events:
        if event["type"] == "message" and not "subtype" in event:
            user_id, message = parse_direct_mention(event["text"])
            if user_id == merakibot_id:
...
if __name__ == "__main__":
    if slack_client.rtm_connect(with_team_state=False):
        print("Meraki Bot connected and running!")
        # Read bot's user ID by calling Web API method `auth.test`
        merakibot_id = slack_client.api_call("auth.test")["user_id"]
        while True:
